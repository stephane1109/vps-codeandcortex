...
# - APP_TICKET_MAX_ACTIVE=1 pour garder l'application sur un seul utilisateur a la fois
# - APP_TICKET_COST=2 pour refleter la charge audio + Whisper
# - CAPACITE_SERVEUR=6 pour coller a la capacite globale du VPS
# - APP_TICKET_TTL_SECONDS=3600 si une transcription longue doit garder son ticket
# - APP_TICKET_ACTIVE_STALE_SECONDS=900 pour nettoyer un ancien ticket actif sans heartbeat apres 15 minutes
# - APP_DATA_DIR=/data/app pour les fichiers temporaires cote serveur
# - WHISPER_MODEL_NAME=small pour conserver le comportement d'origine
# - WHISPER_LANGUAGE=fr pour conserver la langue de transcription d'origine
