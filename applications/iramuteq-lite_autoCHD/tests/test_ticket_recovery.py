...
class TicketRecoveryTests(TestCase):
    def setUp(self) -> None:
        ...
        self.config = {
            "enabled": True,
            "app_id": "iramuteq-lab-test",
            "app_label": "IRaMuTeQ Lab test",
            "max_active": 1,
            "cost": 1,
            "global_capacity": 2,
            "ttl_seconds": 300,
    ...
    def test_header_can_cancel_its_recovered_running_analysis(self) -> None:
        ticket_id = self.claim_ticket()
        self.client.cookies.clear()

        with patch(
            "webapp.main.runtime.cancel_python_analysis",
            return_value={"success": True, "cancelled": True, "jobId": "web-ticket", "message": "Analyse annulée."},
        ):
            response = self.client.post(
                "/api/analysis/abandon",
                headers={ticket_gate.TICKET_ID_HEADER_NAME: ticket_id},
                json={"jobId": "web-ticket", "reason": "Test de récupération."},
            )

        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json()["ticket"]["released"])
        self.assertEqual(response.json()["ticket"]["active"], 0)
