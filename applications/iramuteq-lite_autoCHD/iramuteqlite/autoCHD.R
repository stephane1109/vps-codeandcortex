...
construire_grille_discrimination_simple_iramuteq <- function(config_base) {
  ...
  if (identical(search_profile, "ciblee")) {
    ...
    # Auto discriminante utilise toujours NOM + VER, sans ETRE, en conservant
    # les formes réellement hors lexique sous la catégorie AUTRE_FORME.
    # Les catégories connues non sélectionnées, comme les pronoms, restent
    # exclues : AUTRE_FORME ne doit pas les réintroduire.
            ...
            for (mincl_candidate in mincl_values) {
              for (min_docfreq in min_docfreq_values) {
                for (k_max_candidate in k_max_values) {
                  ...
                  config_variant$iramuteq_discrimination_simple_score_mode <- config_base$iramuteq_discrimination_simple_score_mode %||% "afc_classes_direct"
