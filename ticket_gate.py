...
def _ticket_timeout_seconds(cfg: dict[str, Any], status: str) -> int:
    ...
    if status == "actif" and cfg.get("active_stale_seconds", 0) > 0:
...
def _install_release_hooks(cfg: dict[str, Any], session_id: str | None) -> None:
    ...
    components.html(
        ...
        (function () {{
          ...
          function installReleaseListeners(targetWindow, targetDocument) {{
            if (!targetWindow) {{
              return;
            }}
          ...
          installReleaseListeners(window, document);
          try {{
            if (window.parent && window.parent !== window) {{
              installReleaseListeners(window.parent, window.parent.document);
            }}
          }} catch (error) {{}}
          try {{
            if (window.top && window.top !== window && window.top !== window.parent) {{
              installReleaseListeners(window.top, window.top.document);
            }}
          }} catch (error) {{}}
        }})();
        </script>
