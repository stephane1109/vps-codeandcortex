...
chdrainette_afc_rectangles_overlap <- function(first, second) {
  !(first$xmax < second$xmin || second$xmax < first$xmin ||
    first$ymax < second$ymin || second$ymax < first$ymin)
}

# Même logique que l'anti-chevauchement AFC d'IRaMuTeQ-lite :
# placement séquentiel, test de collision rectangle, puis déplacement en spirale.
chdrainette_afc_place_labels <- function(x, y, labels, sizes, max_iterations = 220L, fixed_rectangles = list()) {
  x <- as.numeric(x)
  y <- as.numeric(y)
  ...
  if (length(sizes) == 1L) sizes <- rep(sizes, label_count)
  sizes <- rep_len(as.numeric(sizes), label_count)
  ...
  # Même pas relatif qu'IRaMuTeQ-lite : proportion de l'étendue du graphe.
  step <- 0.012 * max(range_x, range_y)
  placed <- fixed_rectangles

  for (index in seq_len(label_count)) {
    ...
    if (!is.finite(width) || width == 0) width <- 0.02 * range_x
    if (!is.finite(height) || height == 0) height <- 0.02 * range_y

    label_x <- result_x[index]
    label_y <- result_y[index]
    rectangle <- list(
      xmin = label_x - width / 2,
      xmax = label_x + width / 2,
      ymin = label_y - height / 2,
      ymax = label_y + height / 2
    )

    collision <- FALSE
    if (length(placed)) {
      for (previous in placed) {
        if (!is.null(previous) && chdrainette_afc_rectangles_overlap(rectangle, previous)) {
          collision <- TRUE
          break
        }
      }
    ...
    if (!collision) {
      placed[[length(placed) + 1L]] <- rectangle
      result_rectangles[[index]] <- rectangle
      next
    }
    ...
    for (iteration in seq_len(max_iterations)) {
      angle <- angle + 0.65
      radius <- radius + step * 0.15
      candidate_x <- x[index] + radius * cos(angle)
      candidate_y <- y[index] + radius * sin(angle)
      candidate <- list(
      ...
      if (length(placed)) {
        for (previous in placed) {
          if (!is.null(previous) && chdrainette_afc_rectangles_overlap(candidate, previous)) {
...
chdrainette_plot_afc_classes <- function(afc) {
  ...
  graphics::text(coords[, 1L], coords[, 2L], labels = rownames(coords), pos = 3, cex = 0.95, col = colors[rownames(coords)])
}
...
chdrainette_plot_afc_terms <- function(afc, top_terms = 80L, size_by = "Chi2", avoid_overlap = TRUE) {
  ...
  graphics::points(label_x, label_y, pch = 16, cex = 0.45, col = term_colors)
